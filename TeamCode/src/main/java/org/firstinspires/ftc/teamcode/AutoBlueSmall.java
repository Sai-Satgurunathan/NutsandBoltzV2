...
@Autonomous(name="MoveBackwardTurnLaunchAuto", group="StarterBot")
public class AutoBlueSmall extends OpMode {

    // Feeder timing
    final double FEED_TIME = 0.20; // seconds feeder runs per shot
    final double TIME_BETWEEN_SHOTS = 2.0; // seconds between shots

    // Launcher speeds (RPM/velocity units expected by DcMotorEx - keep units consistent with your config)
    final double LAUNCHER_TARGET_VELOCITY = 1350;
    final double LAUNCHER_MIN_VELOCITY = 1300; // slightly below target as threshold
    ...
    // Autonomous state machine implementing your pseudocode
    private enum AutoState {
        ...
        ROTATE_RIGHT_45,
        LAUNCH_SEQUENCE,
        AFTER_LAUNCH_FORWARD_2FT,
        ROTATE_RIGHT_45_AGAIN,
        MOVE_BACK_6FT,
        COMPLETE
    }
    ...
    public void init() {
        // initial states
        autoState = AutoState.MOVE_BACK_9FT;
        ...
        try {
            ...
            // Some control modules may not support setting PIDF via this call on all hardware; ignore if unsupported.
        }
    ...
    public void loop() {
        switch (autoState) {
            case MOVE_BACK_9FT:
                // Move backward 9 ft = -108 inches
                if (drive(DRIVE_SPEED, -108, DistanceUnit.INCH, 0.75)) {
                    leftDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                    rightDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
            ...
            case FORWARD_2FT:
                // Forward 2 ft = 24 inches
                if (drive(DRIVE_SPEED, 24, DistanceUnit.INCH, 0.5)) {
                    leftDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                    rightDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                    autoState = AutoState.ROTATE_RIGHT_45;
                }
                break;

            case ROTATE_RIGHT_45:
                if (rotate(ROTATE_SPEED, 45, AngleUnit.DEGREES, 0.5)) {
                    leftDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                    rightDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                    // prepare to shoot
                    shotsToFire = 3;
            ...
            case LAUNCH_SEQUENCE:
                // Use launch(boolean) to spin-up and feed. We call with true when we want to start a shot.
                // We request the first shot with true, subsequent calls use false until the shot completes.
                if (launch(shotsToFire > 0)) {
                    // one shot has completed
                    shotsToFire -= 1;
                    if (shotsToFire > 0) {
                        // request next shot on next loop
                        // don't change state; launch(true) will be called next iteration because shotsToFire > 0 in param
                    } else {
                        // finished shooting
                        ...
                        // reset encoders before next drive
                        leftDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                        rightDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                        autoState = AutoState.AFTER_LAUNCH_FORWARD_2FT;
                    }
                }
                break;

            case AFTER_LAUNCH_FORWARD_2FT:
                if (drive(DRIVE_SPEED, 24, DistanceUnit.INCH, 0.5)) {
                    leftDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                    rightDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                    autoState = AutoState.ROTATE_RIGHT_45_AGAIN;
                }
                break;

            case ROTATE_RIGHT_45_AGAIN:
                if (rotate(ROTATE_SPEED, 45, AngleUnit.DEGREES, 0.5)) {
                    leftDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                    rightDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                    autoState = AutoState.MOVE_BACK_6FT;
                }
                break;

            case MOVE_BACK_6FT:
                // Move backward 6 ft = -72 inches
                if (drive(DRIVE_SPEED, -72, DistanceUnit.INCH, 0.75)) {
     ...
     * spinning up the launcher and then feed when velocity threshold is reached. It returns true
     * for one cycle after a ball has been successfully launched.
     * @param shotRequested true if we want to request the next shot
     * @return true for one cycle after a ball has been launched
     */
