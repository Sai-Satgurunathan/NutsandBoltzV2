...
@Autonomous(name="MoveBackwardTurnLaunchAutoReversed", group="StarterBot")
public class AutoRedSmall extends OpMode {
    ...
    // Autonomous state machine (sequence with turns reversed)
    private enum AutoState {
        MOVE_BACK_9FT,
        ROTATE_LEFT_90,         // reversed
        FORWARD_2FT,
        ROTATE_LEFT_45,         // reversed
        LAUNCH_SEQUENCE,
        AFTER_LAUNCH_FORWARD_2FT,
        ROTATE_LEFT_45_AGAIN,   // reversed
        MOVE_BACK_6FT,
        COMPLETE
    }
    ...
    public void init() {
        ...
        telemetry.addData("Status", "Initialized (Reversed Turns)");
        telemetry.update();
    }
    ...
    public void init_loop() {
        ...
        telemetry.addData("Waiting", "Press START to run reversed auto");
        telemetry.update();
    }
        ...
        switch (autoState) {
            case MOVE_BACK_9FT:
                ...
                if (drive(DRIVE_SPEED, -108, DistanceUnit.INCH, 0.75)) {
                    leftDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                    rightDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                    autoState = AutoState.ROTATE_LEFT_90;
                }
                break;

            case ROTATE_LEFT_90:
                // reversed: left turn -> negative angle relative to previous convention
            ...
            case FORWARD_2FT:
                ...
                if (drive(DRIVE_SPEED, 24, DistanceUnit.INCH, 0.5)) {
                    leftDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                    rightDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                    autoState = AutoState.ROTATE_LEFT_45;
                }
                break;
            ...
            case LAUNCH_SEQUENCE:
                // Request shots: call launch(true) to start each shot, subsequent loops continue the sequence.
                if (launch(shotsToFire > 0)) {
                    // one shot has completed
                    shotsToFire -= 1;
                    if (shotsToFire > 0) {
                        // next shot will be requested automatically on next loop
                    } else {
                        // finished shooting
            ...
            case AFTER_LAUNCH_FORWARD_2FT:
                if (drive(DRIVE_SPEED, 24, DistanceUnit.INCH, 0.5)) {
                    leftDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                    rightDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                    autoState = AutoState.ROTATE_LEFT_45_AGAIN;
                }
                break;

            case ROTATE_LEFT_45_AGAIN:
                if (rotate(ROTATE_SPEED, -45, AngleUnit.DEGREES, 0.5)) {
                    leftDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                    rightDrive.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
            ...
            case COMPLETE:
                ...
                leftFeeder.setPower(0);
                rightFeeder.setPower(0);
                telemetry.addData("Auto", "Complete (Reversed Turns)");
                break;
        }
